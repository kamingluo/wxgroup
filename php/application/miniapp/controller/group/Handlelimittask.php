...
class Handlelimittask
{
    ...
    public function pushlimittask(Request $request)
    {
      ...
      if($pages){
          ...
          $dballtasklists = db()->table(array('corwd_limit_task_record'=>'t1','user'=>'t2','crowd_limit_tasks'=>'t3'))->field('t1.*,t2.nickName,t2.avatarUrl,t3.title')->where('t1.user_id=t2.id')->where('t1.crowd_id',$crowd_id)->where('t1.limit_id=t3.id')->where('t2.nickName','like',$newkeyword)->order('id desc')->limit($newpages,20)->select();
      }
      else{
          //兼容没有传页数，返回全部信息
          $dballtasklists = db()->table(array('corwd_limit_task_record'=>'t1','user'=>'t2','crowd_limit_tasks'=>'t3'))->field('t1.*,t2.nickName,t2.avatarUrl,t3.title')->where('t1.user_id=t2.id')->where('t1.crowd_id',$crowd_id)->where('t1.limit_id=t3.id')->where('t2.nickName','like',$newkeyword)->order('id desc')->select();
      }

      $count =db()->table(array('corwd_limit_task_record'=>'t1','user'=>'t2','crowd_limit_tasks'=>'t3'))->field('t1.*,t2.nickName,t2.avatarUrl,t3.title')->where('t1.user_id=t2.id')->where('t1.crowd_id',$crowd_id)->where('t1.limit_id=t3.id')->where('t2.nickName','like',$newkeyword)->count();
