...
class Score
{
    ...
    public function scoreranking(Request $request)
    {
      ...
      $sql = "select user.nickName,user.avatarUrl,user_crowd.score,user_crowd.id from user,user_crowd where user.id=user_crowd.user_id and user_crowd.crowd_id = ".$crowd_id." order BY user_crowd.score desc,user_crowd.id ASC LIMIT ".$startnumber.",20;";
    ...
    public function usercrowdscore(Request $request){
      ...
      $sql="select count(1) as userrank from user_crowd  where  crowd_id = ".$crowd_id." and score >= (select score from user_crowd where user_id = ".$user_id." and crowd_id=".$crowd_id." order BY id DESC )";
